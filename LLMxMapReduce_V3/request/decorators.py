    ...
    def token_logger(self, *args, **kwargs):
        ...
        for count, token_usage in enumerate(cls._token_usage_history):
            if token_usage is None:
                ...
                token_usage_info = (
                    f"(model={token_usage.get('model', None)}, "
                    f"completion_tokens={token_usage.get('completion_tokens', None)}, "
                    f"prompt_tokens={token_usage.get('prompt_tokens', None)}, "
                    f"total_tokens={token_usage.get('total_tokens', None)})"
                )
