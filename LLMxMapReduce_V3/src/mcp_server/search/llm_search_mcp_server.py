...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(log_dir, 'search_server.log'), encoding='utf-8'),
...
DEFAULT_TOPIC_EXPANSION_PROMPT = (
    "You are a professional academic research analysis expert. The user will provide you with a research topic. "
    "Please expand this topic and provide a detailed research description.\n\n"
    "Analyze the topic from multiple perspectives and generate a professional and comprehensive description. "
    "This description will be used for subsequent literature search.\n\n"
...
async def list_tools() -> List[Tool]:
    return [
        Tool(
            ...
            description="Expand the research topic into a detailed description using the LLM.",
            inputSchema={
                "type": "object",
                "properties": {
                    "topic": {
                        "type": "string",
                        "description": "Primary research topic that may require expansion"
                    },
                    "description": {
                        "type": "string",
                        "description": "Optional short description provided by user"
                    },
                    "model": {
                        "type": "string",
                        "description": "Override model identifier (optional)"
                    }
                },
                "required": ["topic"]
            }
        ...
        Tool(
            ...
            description="Ask the human operator for clarification or confirmation and return their response.",
            inputSchema={
                "type": "object",
                "properties": {
                    "question": {
                        "type": "string",
                        "description": "Question or message to display to the human operator"
                    },
                    "context": {
                        "type": "string",
                        "description": "Additional context for the operator (optional)"
                    },
                    "allow_empty": {
                        "type": "boolean",
                        "description": "Whether an empty response is acceptable",
                        "default": True
                    }
                },
                "required": ["question"]
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        if name == "topic_expansion":
            result = await _topic_expansion(
                arguments["topic"],
                arguments.get("description", ""),
                arguments.get("model")
            )
        elif name == "request_human_feedback":
            result = await _request_human_feedback(
                question=arguments["question"],
                context=arguments.get("context", ""),
                allow_empty=arguments.get("allow_empty", True),
            )
...
async def _topic_expansion(topic: str, description: str = "", model: str = None) -> Dict[str, Any]:
    logger.info(f"Starting topic expansion for: {topic}")

    if model is None:
        model = SERVER_CONFIG.get("default_model")
    infer_type = SERVER_CONFIG.get("default_infer_type", "OpenAI")

    prompt_template = SERVER_CONFIG.get("prompts", {}).get(
        "analyse_topic_expansion",
        DEFAULT_TOPIC_EXPANSION_PROMPT,
    )
    prompt = prompt_template.format(topic=topic, description=description or "None")

    request_wrapper = RequestWrapper(model=model, infer_type=infer_type)
    loop = asyncio.get_event_loop()
    timeout_seconds = SERVER_CONFIG.get("llm_request_timeout", 30)

    used_fallback = False
    try:
        expand_func = functools.partial(request_wrapper.completion, prompt)
        expanded_topic = await asyncio.wait_for(
            loop.run_in_executor(None, expand_func),
            timeout=timeout_seconds
        )
    except asyncio.TimeoutError:
        logger.error(f"Topic expansion timed out after {timeout_seconds} seconds for topic: {topic}")
        expanded_topic = description or f"Topic: {topic}"
        used_fallback = True
    except Exception as e:
        logger.error(f"Error during topic expansion: {e}")
        expanded_topic = description or f"Topic: {topic}"
        used_fallback = True

    expanded_topic = (expanded_topic or "").strip()
...
async def _request_human_feedback(question: str, context: str = "", allow_empty: bool = True) -> Dict[str, Any]:
    ...
    def prompt_user() -> str:
        ...
        print("Human feedback requested by MCP tool.", flush=True)
        print(f"Question: {question}", flush=True)
        if context:
            print(f"Context: {context}", flush=True)
        print("Please enter your response (press Enter to submit).", flush=True)
        if not allow_empty:
            print("Empty response is not permitted.", flush=True)
        try:
    ...
    # human_response = await loop.run_in_executor(None, prompt_user)
    human_response = prompt_user()

    status = "response_received"
    if not human_response:
        if allow_empty:
            status = "no_response_provided"
        else:
            human_response = ""
            status = "empty_response_not_allowed"
...
async def _generate_abstract_llm(content: str, url: str, file_path: str, url_index: int) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"LLM summary generation timed out after {abstract_timeout} seconds: {url}")
...
def _load_queries_from_file(query_file_path: str = None, topic: str = "") -> List[str]:
    ...
    try:
        ...
        if query_file_path is None:
            cache_dir = SERVER_CONFIG.get("query_cache_dir", f"output/{datetime.datetime.now().strftime('%Y%m%d')}/{topic}/query_cache")
...
def _load_urls_from_file(url_file_path: str = None, topic: str = "") -> List[str]:
    ...
    try:
        ...
        if url_file_path is None:
            cache_dir = os.path.join(os.path.dirname(__file__), f'../../../output/{datetime.datetime.now().strftime("%Y%m%d")}/{topic}/url_cache')
            if not os.path.exists(cache_dir):
...
def _save_urls_to_file(urls: List[str], topic: str, queries: List[str]) -> str:
    ...
    try:
        ...
        save_dir = os.path.join(os.path.dirname(__file__), f'../../../output/{datetime.datetime.now().strftime("%Y%m%d")}/{topic}/url_cache')
        os.makedirs(save_dir, exist_ok=True)
