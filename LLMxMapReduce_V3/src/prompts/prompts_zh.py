...
# crawl4ai prompts
CRAWL_FILTER_PROMPT_WITH_TOPIC="""分析并处理以下与‘{topic}’相关的网页内容。首先筛选出主体文本，去除超链接、图片链接和广告等无关内容。然后评估筛选后内容的质量和相关性。请按照以下格式提供你的分析：

1. 首先提供筛选后的主要内容，仅保留相关文本和标题。不要进行任何总结。
2. 然后根据以下标准评估筛选后的内容：
    - 与‘{topic}’的相关性：内容与该主题的契合度或对该主题的拓展程度如何？
    - 内容质量和可用性：考虑以下因素：
        * 文本长度和完整性
        * 无乱码字符
        * 整体写作质量
...
请使用以下标签来包含你的回复：
- 标题：<TITLE>你的标题</TITLE>
- 筛选后的内容：<CONTENT>筛选后的文本</CONTENT>
- 质量评分：<SCORE>0 到 100 分</SCORE>

你的回复应保留所有信息性内容，且不包含任何网址，也不要进行总结。
"""
...
# LLM_search prompts
QUERY_EXPAND_PROMPT_WITH_ABSTRACT="""您是一名行业研究专家，负责就 {topic} 的主题撰写一份综合报告。报告应遵守以下要求：{abstract}。要收集必要的信息，您需要进行在线调研。请生成 {query_count} 个搜索查询，帮助您检索报告的相关数据和见解。将当前查询中模糊的概念分解为更具体的子概念，以实现更精确的搜索。例如，“外国”可以进一步分解为在报告领域内具有代表性的特定国家或地区。输出的内容必须被 ```markdown\n``` 引用。
...
```
"""
QUERY_EXPAND_PROMPT_WITHOUT_ABSTRACT="""您是一名行业研究专家，负责就 {topic} 的主题撰写一份综合报告。要收集必要的信息，您需要进行在线调研。请生成 {query_count} 个搜索查询，帮助您检索报告的相关数据和见解。将当前查询中模糊的概念分解为更具体的子概念，以实现更精确的搜索。例如，“外国”可以进一步分解为在报告领域内具有代表性的特定国家或地区。 输出的内容必须被 ```markdown\n``` 引用。
