...
# crawl4ai prompts
PAGE_REFINE_PROMPT = """Analyze and process the following web content related to '{topic}'. Output the main text, removing image links, URL links, advertisements, meaningless repeated characters, etc. Do not summarize the content, all information related to the topic should be retained.
...
SIMILARITY_PROMPT = """Please judge the quality score of this content based on the following topic and content retrieved from the internet.

Topic: {topic}
Retrieved content: {content}

Please score the retrieved content based on the following dimensions. Please be as strict and critical as possible in scoring.

1. The relevance of the content to the topic. This needs to consider whether the content can be viewed as part of the topic's sub-content for expansion.
2. The quality of the content that can be used to write text related to the topic. This needs to consider the length of the text (for example: if the length is very short, the reference value is relatively low), whether the text contains a lot of garbled characters, the quality of the text itself, etc.

Please comprehensively consider the above two dimensions, first give the reason for scoring, then score. You need to score each dimension, with a scoring range of 0-100. 0 means completely irrelevant, 100 means completely relevant. After completing the scoring for each dimension, you need to calculate and get the final average score.
