...
class Digest:
    ...
    def all_content(self, with_title=False, with_index=False):
        all_section = self.root.all_section if hasattr(self.root, 'all_section') else []
    ...
    @classmethod
    def from_json(cls, json_str: str, outline=None) -> T:
        data = json.loads(json_str)
        
        instance = cls([], data["survey_title"])
        instance.paper_infos = data["paper_infos"]
        instance.suggestions = data["suggestions"]
        instance.failure_count = data["failure_count"]
        
        if data["root"]:
            instance.root = cls._deserialize_root(data["root"])
            instance.root.update_section()
            
            if outline:
                instance.find_matching_section(instance.root, outline)
    ...
    @staticmethod
    def _deserialize_root(data: Dict[str, Any]) -> 'DigestNode':
        if not data:
            return None
            
        def deserialize_node(node_data):
            node = DigestNode(node_data["title"], node_data["description"])
            node.sons = [deserialize_node(son_data) for son_data in node_data["sons"]]
            for son in node.son:
                son.father = node
            return node
