...
POLISH_PROMPT = """[任务描述] 
1. 将多个连续的引用, 如 ([BIBKEY1], [BIBKEY2]), 转换为这种形式：["BIBKEY1", "BIBKEY2"]，移除包裹他们的圆括号或方括号，以及重复的多个引用。输出内容中的所有形式必须为 ["BIBKEY1", "BIBKEY2"]，禁止添加多余的圆括号，方括号，以及将多个引用分散在不同的方括号中。
2. 将输出中的其他语言转为汉语。内容中除了有特定含义的词汇或缩写，其他所有语言必须为汉语，不允许混杂除汉语与英语之外的其他语言。
