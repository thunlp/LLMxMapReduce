...
2. 每组参考文献的bibkeys必须用一对括号括起来。引用具体文章的bibkey，而不是使用“所有文章”或“所有部分”等通用术语。引用文章的bibkey，而不是文章标题。如果描述中没有合适的文章可引用，则写句子时不带任何引用。不要在句子末尾留空括号[]。
...
- **Digest Analysis**：解释如何使用提取的信息来组织和分析文章，并提供可执行的步骤。避免仅仅列出信息，而是分析和综合这些信息，形成一个连贯且结构良好的叙述。例如，提取共同模式、冲突或演化趋势（如“由于数据集偏差，方法X在研究A和B中产生了不同的结果”），提出代表性观点（如“虽然主流研究强调因素Y，但新兴研究质疑其长期有效性”），提供文献综述写作的可操作指导，例如：“比较研究A（2018年）和研究B（2022年）的实验设计，以解释结论分歧的潜在原因。”，“总结表2中7项研究的共同局限性，并提出改进框架。”，并突出未解决的问题或跨学科机会（如“将计算模型X与实证方法Y结合可以克服当前的瓶颈”）。
...
2. 每个部分的描述应引用适当的文章bibkeys。如果你认为某个部分的内容可以借鉴某些摘要，则应在句子末尾包含相应的bibkeys。
3. 每个部分必须包含适当的子部分，建议使用Markdown标题来表示层次结构。
4. 每组参考文献的bibkeys必须用一对括号括起来。引用具体文章的bibkey，而不是使用“所有文章”或“所有部分”等通用术语。引用文章摘要的bibkeys，而不是索引本身。如果描述中没有合适的文章可引用，则写句子时不带任何引用。
...
2. 每个部分的描述应引用适当的文章bibkeys。如果你认为某个部分的内容可以借鉴某些摘要，则应在句子末尾包含相应的bibkeys。  
3. 每个部分必须包含适当的子部分，建议使用Markdown标题来表示层次结构。  
4. 每组参考文献的bibkeys必须用一对括号括起来。引用具体文章的bibkey，而不是使用“所有文章”或“所有部分”等通用术语。引用初始大纲描述中提到的文章，而不是索引本身。如果描述中没有合适的文章可引用，则写句子时不带任何引用。  
...
2. 每组参考文章的bibkey必须用一对括号括起来。引用特定文章的bibkey，而不是使用“所有文章”或“所有章节”之类的通用术语。引用在初步大纲和文章摘要描述中提到的文章的bibkey，而不是索引本身。如果描述中没有合适的文章引用，请不加引用，直接写出句子。
...
## 格式要求：
1. 所有建议必须用一对```suggestion\n```引用。不要在输出中嵌套```suggestion\n```。不要用多个```suggestion\n```包围多个建议。
2. 每组参考文章的bibkey必须用一对括号括起来。引用特定文章的bibkey，而不是使用“所有文章”或“所有章节”之类的通用术语。引用在初步大纲和个别建议描述中提到的文章的bibkey，而不是索引本身。如果描述中没有合适的文章引用，请不加引用，直接写出句子。
...
- **Digest Analysis**：解释如何使用提取的信息来组织和分析文章，并提供可执行步骤。避免仅列出信息；应对信息进行分析和综合，形成一个连贯且结构良好的叙述。例如，提取常见模式、冲突或演变趋势（如：“方法X在研究A和B中由于数据集偏差得出不同结果”），提出具有代表性的观点（如：“虽然主流研究强调因子Y，但新兴研究质疑其长期有效性”），为文献综述写作提供可执行指导，如：“比较研究A（2018）和研究B（2022）的实验设计，以解释得出不同结论的潜在原因。”、“总结表2中7项研究的共同局限性并提出改进框架。”，并突出未解决的问题或跨学科的机会（如：“将计算模型X与经验方法Y相结合可能克服当前瓶颈”）。
...
4. 每组参考文章的bibkey必须用一对括号括起来。引用特定文章的bibkey，而不是使用“所有文章”或“所有章节”之类的通用术语。引用在初步大纲和修改建议描述中提到的文章的bibkey，而不是索引本身。如果描述中没有合适的文章引用，请不加引用，直接写出句子。
...
2. 每组参考文章的bibkey必须用一对括号括起来。引用特定文章的bibkey，而不是使用“所有文章”或“所有章节”之类的通用术语。引用在子章节描述和文章摘要描述中提到的文章的bibkey，而不是索引本身。如果描述中没有合适的文章引用，请不加引用，直接写出句子。不要单独列出所有bibkey，而是将它们整合到内容中。
...
输出要求：
1. 输出的章节内容必须用一对```markdown\n```引用。
2. 每组参考文章的bibkey必须用一对括号括起来。引用特定文章的bibkey，而不是使用“所有文章”或“所有章节”之类的通用术语。引用在章节描述和文章摘要描述中提到的文章的bibkey，而不是索引本身。如果描述中没有合适的文章引用，请不加引用，直接写出句子。不要单独列出所有bibkey，而是将它们整合到内容中。
3. 如果输出中包含公式，请使用LaTeX格式表示。例如，内联公式使用$y = x^2$，块状公式使用$$y = x^2$$。
====================
格式示例：
```markdown
...
4. 一个章节可以使用多个图表，并且不是每个章节都需要用图表表示。
...
2. 位置句子。重复与图表最相关的句子。该图将放置在这个句子之后。
3. 图表标题，概括该图表的主要内容。
4. 用 ```mermaid\\n``` 引用的 Mermaid 代码。
- 严格遵守 Mermaid 语法。
- 每个节点标签必须用 [""] 引用。
...
5. 每组参考文献 **bibkey** 必须用方括号 `[]` 括起来，并且你必须引用 **初始大纲** 和 **修改建议** 中的文献。**不要直接引用"初始大纲"和"建议"**。当引用多个**bibkey**时，应将它们一起放入方括号 `[]` 中。必须引用具体的论文，而不能使用“所有论文”、“所有部分”等笼统术语。如果没有合适的文献提供，你不能添加新的引用。多个引用的 **bibkey** 应该放在同一对方括号 `[]` 内。  
...
# LLM_search prompts
SEARCH_SYSTEM_PROMPT_WITH_ABSTRACT="""你是一个行业研究专家，需要写一篇报告，报告的主题是{topic}，要求是{abstract}。现在你要在互联网上查阅相关资料，需要写一些中文的检索query。
"""
SEARCH_SYSTEM_PROMPT_WITHOUT_ABSTRACT="""你是一个行业研究专家，需要写一篇报告，报告的主题是{topic}。现在你要在互联网上查阅相关资料，需要写一些中文的检索query。
"""
SEARCH_USER_PROMPT="""如果query中包含比较笼统、模糊的概念，请你将这些概念拆解为多个子概念，便于更精确地搜索，例如“国外”可以进一步拆解为该报告涉及领域里比较有代表性的具体国家和地区。请你仅用','隔开query与query并以简单字符串的形式返回给我。注意只返回我要的query，不要输出其他的任何东西。
"""
...
# crawl4ai prompts
CRAWL_FILTER_PROMPT_WITH_TOPIC="""下面是一份用爬虫工具获取的网页信息。请你定位正文部分，并且过滤掉与正文无关的内容（包括文字上的超链接和文字间的图片链接、广告等）。请你直接返回过滤后的文字（包含标题），无需输出其他任何信息。\n这个网页的正文内容与\"{topic}\"相关。\n
"""
...
"""
GENERATE_TITLE_PROMPT="""请根据以下内容，生成一个简短的标题：\n{text}\n请确保标题简洁明了，能够概括文章的主要内容。你需要将返回的标题嵌套在
markdown\n
代码块中。例如:
markdown
标题
